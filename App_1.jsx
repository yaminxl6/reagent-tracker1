...
export default function App() {
  ...
  // A plain setTimeout isn't reliable here: most phones dim/lock the screen
  // after a minute or two even if the person never pressed the lock button,
  // and browsers suspend JS timers while the screen is off. So instead we
  // record the real clock time of the last activity in localStorage, and
  // check elapsed time whenever the tab becomes visible/focused again (plus
  // a periodic check while it's open) — that catches the "screen was off"
  // case that a bare timer would miss.
  useEffect(() => {
    if (!role) return;
    const TIMEOUT_MS = 30 * 60 * 1000;
    const STORAGE_KEY = "reagent_last_activity";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F0F3F2", fontFamily: "'IBM Plex Sans', sans-serif", color: "#1B2B2E" }}>
      ...
      <main style={{ maxWidth: 980, margin: "0 auto", padding: "24px 20px 80px" }}>
        ...
        {tab === "labmap" && <LabMap />}
        {tab === "bloodbank" && <BloodBankDisposal username={username} />}
  ...
  return (
    <aside className={className}>
      ...
      <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, padding: "14px 12px", overflowY: "auto" }}>
        ...
        <SideBtn active={tab === "bloodbank"} onClick={() => setTab("bloodbank")} icon={<Droplet size={16} />} label="Blood Bank disposal" />
